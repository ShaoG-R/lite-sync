...
/// Benchmark: Oneshot creation comparison (custom Generic vs tokio oneshot)
/// 基准测试：Oneshot 创建对比（自定义 Generic vs tokio oneshot）
fn bench_oneshot_creation_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("oneshot_generic_creation");
    
    // Custom oneshot - Generic
    group.bench_function("lite_sync_generic", |b| {
        b.iter(|| {
...
/// Benchmark: Oneshot send/recv comparison (custom Generic vs tokio oneshot)
/// 基准测试：Oneshot 发送接收对比（自定义 Generic vs tokio oneshot）
fn bench_oneshot_send_recv_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("oneshot_generic_send_recv");
...
fn bench_oneshot_batch_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("oneshot_generic_batch");
    
    for size in [10, 100, 1000].iter() {
        // Custom oneshot - Generic
        group.bench_with_input(
            BenchmarkId::new("lite_sync_generic", size),
            size,
            |b, &size| {
                let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_oneshot_cross_task_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("oneshot_generic_cross_task");
...
fn bench_oneshot_immediate_notification(c: &mut Criterion) {
    let mut group = c.benchmark_group("oneshot_generic_immediate");
...
fn bench_oneshot_drop_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("oneshot_generic_drop");
    
    // Custom oneshot - drop without receiving (no waker registered)
    group.bench_function("lite_sync_generic_drop_no_recv", |b| {
        b.iter_batched(
            || {
    ...
    group.bench_function("lite_sync_generic_drop_after_notify", |b| {
        b.iter_batched(
            || {
    ...
    group.bench_function("lite_sync_generic_drop_with_waker", |b| {
        let runtime = tokio::runtime::Runtime::new().unwrap();
    ...
    group.bench_function("lite_sync_generic_batch_drop", |b| {
        b.iter_batched(
            || {
